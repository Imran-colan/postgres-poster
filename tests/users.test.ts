...
import { createArrayComparator } from "@src/common/util/validators";
...
// database
const compareUserArrays = createArrayComparator<IUser>(["name", "email"]);
...
describe("UserRouter", () => {
  ...
  describe(`"GET:${Paths.Users.Get}"`, () => {
    ...
    it(
      "should return a JSON object with all the users and a status code " +
        `of "${HTTP_STATUS_CODES.Ok}" if the request was successful.`,
      async () => {
  ...
  describe(`"POST:${Paths.Users.Add}"`, () => {
    ...
    it(
      `should return a status code of "${HTTP_STATUS_CODES.Created}" if ` +
  ...
  describe(`"PUT:${Paths.Users.Update}"`, () => {
    // Success
    it(
      `should return a status code of "${HTTP_STATUS_CODES.Ok}" if the ` +
    ...
    it(
      "should return a JSON object with the error message of " +
        ...
        `"${HTTP_STATUS_CODES.NotFound}" if the id was not found.`,
      async () => {
